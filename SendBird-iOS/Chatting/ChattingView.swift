...
class ChattingView: ReusableViewFromXib, UITableViewDelegate, UITableViewDataSource, UITextViewDelegate {
   ...
   var delayMenuHidden: Bool = true;
    ...
    func initChattingView() {
      ...
      minField.isHidden = true;
      secField.isHidden = true;
      delayMenuHidden = true;
    }
...
//                            (cell as! OutgoingFileMessageTableViewCell).showMessageControlButton()
   ...
   @IBAction func onMSGSend(_ sender: UIButton) {
      //print("\n\n\n\n\n")
      let payload : String = self.messageTextView.text;
      let min : Int = Int(minField.text!)!
      let sec : Int = Int(secField.text!)!
      print("Delay: " + String(min) + ", " + String(sec))
      let delay : String = String(min * 60 + sec)
      
      
      //print(payload);
      //print(self.messages[self.messages.endIndex-1])
      let parameters = ["recipient": "John Robertson", "payload": payload, "delay": delay]
      print("\n\n\n\n\n\n")
      print(parameters)
      
      /*guard let url = URL(string: "http://71.93.48.198:9000/api/send") else { return }
   ...
   @IBAction func delayButtonPressed(_ sender: UIButton) {
      if delayMenuHidden {
         ...
         minField.isHidden = false
         secField.isHidden = false
         delayMenuHidden = false
      } else {
         delayView.isHidden = true;
         minField.isHidden = true;
         secField.isHidden = true;
